...
'''
    ...
    def _add_widget_objects(self, twx: zipfile.ZipFile):
        ...
        for twx_obj in self.twx_objects:
            ...
            if twx_obj.id in seen_ids:
                logger.debug(f"Skipping duplicate object: {object_path}")
                continue
            seen_ids.add(twx_obj.id)
        ...
        if self.coach_objects:
            ...
            for coach_obj in self.coach_objects:
                ...
                if coach_obj.id in seen_ids:
                    logger.debug(f"Skipping duplicate coach: {object_path}")
                    continue
                seen_ids.add(coach_obj.id)
    ...
    def _replace_project_info(
        ...
        try:
            ...
            if project_elem is not None:
                ...
                project_elem.set('isToolkit', str(self.config.is_toolkit).lower())
                logger.debug(f"Updated project: {self.config.name} ({self.config.short_name}), isToolkit={self.config.is_toolkit}")
            else:
    ...
    def _generate_objects_section(self) -> str:
        ...
        # Add widget objects (deduplicated)
        seen_manifest_ids = set()
        for twx_obj in self.twx_objects:
            if twx_obj.id in seen_manifest_ids:
                continue
            seen_manifest_ids.add(twx_obj.id)
            object_line = f'        <object id="{twx_obj.id}" versionId="{twx_obj.version_id}" name="{escape_xml(twx_obj.name)}" type="{twx_obj.object_type}"/>'
            object_lines.append(object_line)

        # Add coach objects (deduplicated)
        for coach_obj in self.coach_objects:
            if coach_obj.id in seen_manifest_ids:
                continue
            seen_manifest_ids.add(coach_obj.id)
