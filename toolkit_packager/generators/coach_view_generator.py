...
'''
    ...
    def generate_binding_from_config(self, coach_view_id: str, binding_config: dict) -> str:
        ...
        # 1. Check custom type registry first — handles all BO types (e.g. ApproverAction, StepItem)
        custom_registry = get_custom_type_registry()
        custom_type_id = custom_registry.get_type(class_id_raw) if class_id_raw else None
        if custom_type_id:
            class_id = custom_type_id.lstrip('/')
            logger.info(f"Binding '{binding_name}' resolved to custom BO type '{class_id_raw}': {class_id}")
        else:
            # 2. Map primitive type names to BAW class IDs
            if class_id_raw and not class_id_raw.startswith('/') and '/' not in class_id_raw:
                class_id = self.get_default_class_id(class_id_raw)
            else:
                class_id = class_id_raw if class_id_raw else self._add_dependency_prefix('12.db884a3c-c533-44b7-bb2d-47bec8ad4022')

            # 3. Check if there's a matching business object already generated for this widget
                ...
                if widget_config and 'businessObjects' in widget_config:
                    for bo_ref in widget_config['businessObjects']:
                        ...
                        if bo_name and bo_name in business_objects:
                            ...
                            logger.debug(f"Linked binding '{binding_name}' to business object '{bo_name}': {class_id}")
                            break

            # 4. Infer from config schema as last resort
