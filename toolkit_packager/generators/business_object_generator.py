...
'''
    ...
    def generate_properties_xml(self, properties: list) -> str:
        ...
        for prop in properties:
            ...
            is_list = prop.get('isList', False)
            
            # Map type to BAW class reference
            class_ref = self.get_class_ref_for_type(prop_type)
            array_property = 'true' if is_list else 'false'
            
            prop_xml = f'''            <property>
                <name>{prop_name}</name>
                <description isNull="true" />
                <classRef>{class_ref}</classRef>
                <arrayProperty>{array_property}</arrayProperty>
    ...
    def generate_json_data(self, properties: list) -> str:
        ...
        for prop in properties:
            ...
            # Resolve type URI and refid for jsonData
            type_mappings = load_type_mappings()
            registry = get_custom_type_registry()
            base_type = prop_type.rstrip('[]')
            prim_id = type_mappings.get(base_type) or type_mappings.get(base_type.lower())
            if prim_id:
                # primitive — use lombardi schema
                xsd_type = f"{{http://lombardi.ibm.com/schema/}}{base_type}"
                ref_id = prim_id
            else:
                # custom BO — use CW namespace
                xsd_type = f"{{http://CW}}{base_type}"
                custom_id = registry.get_type(base_type)
                ref_id = custom_id.lstrip('/') if custom_id else "12.db884a3c-c533-44b7-bb2d-47bec8ad4022"
            ...
            element = {
                ...
                "type": xsd_type,
                "otherAttributes": {
                    "{http://www.ibm.com/bpmsdk}refid": ref_id
                }
            }
            if is_list:
